...
class HFGeneration {
	...
	public static void hf_generation(com.nazarov.radman.jaad.aac.sbr.SBR sbr, float[][][] Xlow,
									 float[][][] Xhigh, int ch) {
		int l, i, x;
		...
		if(sbr.Reset) {
			com.nazarov.radman.jaad.aac.sbr.FBT.limiter_frequency_table(sbr);
		}
	}

	private static void auto_correlation(com.nazarov.radman.jaad.aac.sbr.SBR sbr, acorr_coef ac, float[][][] buffer,
										 int bd, int len) {
	...
	private static void calc_prediction_coef(com.nazarov.radman.jaad.aac.sbr.SBR sbr, float[][][] Xlow,
	...
	/* FIXED POINT: bwArray = COEF */
	private static void calc_chirp_factors(com.nazarov.radman.jaad.aac.sbr.SBR sbr, int ch) {
		int i;
