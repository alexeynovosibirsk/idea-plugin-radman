package com.nazarov.jaad.aac;

import com.nazarov.jaad.aac.syntax.BitStream;
import com.nazarov.jaad.aac.syntax.Constants;
import com.nazarov.jaad.aac.syntax.PCE;
...
public class DecoderConfig implements Constants {
	...
	private com.nazarov.jaad.aac.Profile profile, extProfile;
	...
	private DecoderConfig() {
		profile = com.nazarov.jaad.aac.Profile.AAC_MAIN;
		extProfile = com.nazarov.jaad.aac.Profile.UNKNOWN;
	...
	public com.nazarov.jaad.aac.Profile getExtObjectType() {
		return extProfile;
	}

	public void setExtObjectType(com.nazarov.jaad.aac.Profile extObjectType) {
	...
	public com.nazarov.jaad.aac.Profile getProfile() {
		return profile;
	}

	public void setProfile(com.nazarov.jaad.aac.Profile profile) {
	 ...
	 */
	static DecoderConfig parseMP4DecoderSpecificInfo(byte[] data) throws com.nazarov.jaad.aac.AACException {
		...
		try {
			...
			switch(config.profile) {
				...
				case ER_AAC_LD:
					...
					if(config.frameLengthFlag) throw new com.nazarov.jaad.aac.AACException("config uses 960-sample frames, not yet supported"); //TODO: are 960-frames working yet?
					...
					break;
				default:
					throw new com.nazarov.jaad.aac.AACException("profile not supported: "+config.profile.getIndex());
			}
			return config;
		}
		finally {
	...
	private static com.nazarov.jaad.aac.Profile readProfile(BitStream in) throws com.nazarov.jaad.aac.AACException {
		int i = in.readBits(5);
		if(i==31) i = 32+in.readBits(6);
		return com.nazarov.jaad.aac.Profile.forInt(i);
	}

	private static void readSyncExtension(BitStream in, DecoderConfig config) throws com.nazarov.jaad.aac.AACException {
		final int type = in.readBits(11);
		switch(type) {
			case 0x2B7:
				final com.nazarov.jaad.aac.Profile profile = com.nazarov.jaad.aac.Profile.forInt(in.readBits(5));
