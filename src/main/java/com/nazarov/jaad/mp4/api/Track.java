...
import com.nazarov.jaad.mp4.od.DecoderSpecificInfo;
import com.nazarov.jaad.mp4.od.Descriptor;
...
public abstract class Track {
	...
	private final List<com.nazarov.jaad.mp4.api.Frame> frames;
	...
	Track(Box trak, MP4InputStream in) {
		...
		if(stbl.hasChildren()) {
			frames = new ArrayList<com.nazarov.jaad.mp4.api.Frame>();
	...
	private void parseSampleTable(Box stbl) {
		...
		final com.nazarov.jaad.mp4.api.Type type = getType();
		...
		for(int i = 0; i<firstChunks.length; i++) {
			...
			for(int j = (int) firstChunks[i]-1; j<lastChunk; j++) {
				...
				for(int k = 0; k<samplesPerChunk[i]; k++) {
					...
					frames.add(new com.nazarov.jaad.mp4.api.Frame(type, offset, sampleSizes[current], timeStamp));
	 ...
	 */
	public com.nazarov.jaad.mp4.api.Frame readNextFrame() throws IOException {
		com.nazarov.jaad.mp4.api.Frame frame = null;
		if(hasMoreFrames()) {
